...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train a neural network model to classify images of numbers."
    )
    parser.add_argument(
        "--path_to_save",
        type=str,
    ...
    parser.add_argument(
        "--batch_size", type=int, default=32, help="The batch size to use for training."
    )
    parser.add_argument(
        "--num_epochs",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-3, help="The learning rate to use for training."
    )
...
def main():
    ...
    results_df = pd.DataFrame(
        columns=["epochs", "train_acc", "train_loss", "test_acc", "test_loss"]
    )
    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        test_acc, test_loss = test_step(
            model=model, dataloader=test_loader, loss_fn=loss_fn
        )
        row = pd.DataFrame(
            {
                "epoch": [epoch],
    ...
    torch.save(
        {"model_state_dict": model.state_dict(), "class_names": train_dataset.classes},
        path_to_save,
    )
